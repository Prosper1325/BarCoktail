...
class Employe(Logable):
    ...
    def __init__(self, pic, bar, clients, name, verbose=True, productivity=1.0):
        ...
        self.productivity = productivity  # facteur de temps par tâche
    ...
    async def servir(self):
        while True:
            ...
            for conso in commande:
                ...
                await asyncio.sleep(0.3 * self.productivity)  # temps de service dynamique
            await asyncio.sleep(0)  # rend la main
    ...
    async def preparer(self):
        while True:
            ...
            for conso in cmd:
                ...
                await asyncio.sleep(0.4 * self.productivity)  # temps de préparation dynamique
...
async def main_async(fichier):
    ...
    # Bariste avec productivité = 1.0 (normal)
    bob = Bariste(le_pic, le_bar, clients, name="bob", verbose=True, productivity=1.0)
    
    # Deux serveurs avec productivité différente
    alice = Serveur(le_pic, le_bar, clients, name="alice", verbose=True, productivity=1.0)
    prosper = Serveur(le_pic, le_bar, clients, name="prosper", verbose=True, productivity=1.5)  # un peu plus lent
